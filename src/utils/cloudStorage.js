...
// Pro 여부 확인 - AuthContext의 isPro를 사용할 것을 권장
// 레거시 호환을 위해 파라미터로 isPro를 받음
export function isProUser(isPro) {
    return isPro === true;
}

// 유저 타입 반환: 'guest' | 'free' | 'pro'
// isPro 파라미터: AuthContext에서 받은 서버 검증된 Pro 상태
export function getUserTier(user, isPro = false) {
...
export async function addItemCloud(user, item, isPro = false) {
    const newItem = {
    ...
    const tier = getUserTier(user, isPro);
...
export async function migrateLocalToCloud(user) {
    ...
    // 마이그레이션 시에는 일단 free 한도로 제한 (Pro는 이후 서버에서 확인)
    const itemsToMigrate = localItems.slice(0, FREE_USER_ITEM_LIMIT);
...
// ===== 현재 아이템 수 + 한도 정보 =====
export async function getStorageInfo(user, isPro = false) {
    if (!user || user.isAnonymous) {
        const items = JSON.parse(sessionStorage.getItem('guest_items') || '[]');
